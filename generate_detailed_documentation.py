...
def generate_document():
    ...
    add_paragraph(
        doc,
        "1. The Release Set (Training Data): 35 recordings, originally labeled a01-a20 (apnea), b01-b05 (borderline), and c01-c10 (control/healthy).",
    )
    add_paragraph(
        doc,
        "2. The Challenge Set (Testing Data): 35 recordings, labeled x01 through x35. The labels for this set were originally withheld for a PhysioNet competition but are now publicly available.",
    )
    ...
    add_heading(
        doc, "Chapter 5: Architectural Overhaul and PyTorch Transition", level=1
    )
    ...
    add_paragraph(
        doc,
        "The updated SE-MSCNN v2 model was trained on CPU. Preprocessed data was successfully cached via Python's pickle library, reducing iterative startup time from 1.5 hours to 5 seconds. Over 50 epochs, the cosine annealing scheduler functioned exactly as theorized. The final measured metrics are as follows:",
    )
    ...
    add_paragraph(
        doc,
        "- SE-MSCNN_robust_v2.py / related CSVs: Deprecated intermediate scripts attempting to fix the baseline without changing frameworks.",
    )
    add_paragraph(
        doc,
        "- SE-MSCNN_improved_baseline.py: Deprecated scripts regarding the 50-sample overfit subset test.",
    )
    add_paragraph(
        doc,
        "- preprocess_with_spo2.py / train_with_spo2.py: The failed experiments utilizing missing SPO2 data.",
    )
    add_paragraph(
        doc,
        "- Multiple .keras files: Old TensorFlow weights no longer compatible with the PyTorch V2 model.",
    )
    add_paragraph(
        doc,
        "- Various Jupyter Notebooks: Exploratory analysis notebooks (BAFNET, LeNet) that were abandoned early in the timeline.",
    )

    add_heading(doc, "Retained Final Architecture", level=2)
    add_paragraph(
        doc,
        "- SE_MSCNN_v2_improved.py: The master PyTorch training script containing the final 50-epoch configuration.",
    )
    add_paragraph(
        doc,
        "- benchmark_final_model.py: The evaluation script to quickly load cached data and generate confusion matrix plots.",
    )
    add_paragraph(
        doc,
        "- generate_word_report.py: The script used to generate the IEEE metrics summary.",
    )
    add_paragraph(
        doc, "- weights.v2_improved.pt: The fully trained 94.75% model weights."
    )
    add_paragraph(
        doc,
        "- preprocessed_data.pkl: The cached feature data representing 1.5 hours of compiled compute time.",
    )
