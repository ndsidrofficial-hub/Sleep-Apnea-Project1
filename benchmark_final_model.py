...
def run_benchmark():
    ...
    with open("preprocessed_data.pkl", "rb") as f:
        data = pickle.load(f)
    print(
        f"[Dataset] Train: {len(data['y_train'])}, Val: {len(data['y_val'])}, Test: {len(data['y_test'])}"
    )
    ...
    print(
        classification_report(y_test, ensemble_preds, target_names=["Normal", "Apnea"])
    )
    ...
    # ROC Curve
    axes[1].plot(
        fpr, tpr, color="cyan", lw=2, label=f"Ensemble ROC (AUC = {roc_auc:.4f})"
    )
