...
run = title.add_run('Improved Sleep Apnea Detection Using SE-MSCNN v2\nwith XGBoost Ensemble on PhysioNet Apnea-ECG Database')
run.bold = True
run.font.size = Pt(14)
...
run = footnote.add_run('*Test accuracy after 10 epochs (initial run). Best validation accuracy: 94.75% (50 epochs).')
run.font.size = Pt(8)
run.italic = True
...
run = footnote2.add_run('*Temporary dips due to Cosine Annealing Warm Restart (LR reset). Best val acc: 94.75%.')
run.font.size = Pt(8)
run.italic = True
